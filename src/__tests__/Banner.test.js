...
test('it shows the title in the banner', () => {
	setup();
	// We expect that the title 'Hi, Iam Gonzalez' is in the banner component
	expect(screen.getByText(/Hi, Iam Gonzalez/i)).toBeInTheDocument();
});
...
test('can download cv when clicked on download cv button', async () => {
	...
	// expect(downloadLink.download).toEqual('CV-CristianGonzalez.pdf');
	// expect(downloadLink.href).toEqual('/files/CV-CristianGonzalez.pdf');
